    ...
    if selected_city != "Select a City":
        ...
        if not city_forecast_df.empty:
            ...
            if not city_forecast_df.empty:  # ✅ Evita error si `city_forecast_df` está vacío
                num_days = len(city_forecast_df)  # ✅ Número de días en el forecast
                forecast_cols = st.columns(num_days)  # ✅ Crear columnas según el número de días disponibles
                
                for idx, row in enumerate(city_forecast_df.itertuples()):  # ✅ `enumerate()` para indexar correctamente
                    forecast_icon = weather_icons.get(row.weather_condition.strip().lower(), "🌎")
                    with forecast_cols[idx]:  # ✅ Ahora `idx` siempre estará dentro del rango
                        st.markdown(f"""
